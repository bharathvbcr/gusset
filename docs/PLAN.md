...
Two small packages and a generated header; the binding layer underneath is whatever the app already uses.

```mermaid
flowchart TD
  A[Go app code] --> B[Go package<br/>bounded handles, deadlines, poisoning]
  B --> C[cgo bindings<br/>hand-written or uniffi-generated]
  C --> D[cbindgen header<br/>ABI version + sizes]
  D --> E[Rust crate<br/>guard, status, worker pool, alloc stats]
  E --> F[Rust engine<br/>tessl / sparsl / app core]
  E -. pipe completion .-> B
```
...
| --- | --- | --- | --- |
| Runtime crate | Rust | `ffi_guard`, `FfiStatus` (ptr+len, never NUL-terminated), `gusset_status_free`, panic hook with location, ABI layout export, worker pool with explicit stack size, allocator stats, timeout and cancel checks | 14 exported functions, 3 `#[repr(C)]` types (`CallHeader`, `FfiStatus`, `AbiLayout`) |
...
| Header | C | Generated by `cbindgen` from the crate; checked in so `go build` does not need Cargo | one `.h` file |
| Example engine | Rust + Go | Trivial engine that exercises every failure mode: panic, NUL in message, deadline miss, large allocation, deep recursion | reference for adopters |
...
| 1 · v0.1 for the first app | Rust crate + Go package with the six invariants; `cbindgen` header; bounded handle; completion channel; ABI check | The Go service in front of tessl/sparsl runs on it; thread-cap soak (10k blocked calls) fails to crash the process | 3–5 weeks |
