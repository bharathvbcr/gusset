...
4. Public surface stays at 17 exported Rust functions (the list in the specifications is the whole ABI; `gusset_abi_fields` is the fifteenth, `DECISIONS.md` 2026-09-22; `gusset_handle_ring` and `gusset_ring_release` the sixteenth and seventeenth, `DECISIONS.md` 2026-09-26) and 12 Go entry points (`CallBuffer`, `Shutdown`, `WaitBuffer` logged in `DECISIONS.md` 2026-09-20). Adding another needs a line in `DECISIONS.md` saying why.
...
| Equal-width field swap | `flags` and `reserved` trade places, the size check passes, and every later read uses the wrong slot | Version 2 compared `size_of` and `align_of` only. Equal-width fields keep both numbers | `gusset_abi_fields` reports each named field's offset and size, capped by the caller's buffer; Go compares them to cgo `Offsetof`/`Sizeof`; `TestPitfall_EqualWidthFieldSwapKeepsSizeAndBreaksNamedOffsets` |
...
| Live-byte counter wraps near `usize::MAX` | `AdviseMemoryLimit` treats a saturated Rust heap as almost empty and raises the Go limit | `fetch_add` plus `wrapping_add` folded the counter | Saturating CAS; `accounting_is_balanced_saturating_and_peak_monotonic` |
| Ticket and buffer ids wrap onto live ids | A later completion wakes the wrong waiter, or `buf_alloc` replaces a live buffer | `fetch_add` keeps advancing after the 63-bit ceiling, including on the call that returns an error | `reserve_id` refuses without advancing; `buffer_ids_stop_at_the_ceiling_instead_of_wrapping`, `ticket_ids_stop_at_the_ceiling_instead_of_wrapping` |
| Full submission queue blocks the cgo thread | `close` cannot disconnect workers, or a cloned sender keeps the channel open across `join` | `SyncSender::send` waits; releasing the lock before the send reopens the cancel-flag leak | `try_send` under the sender lock; `submit_on_a_full_queue_fails_without_blocking_or_leaking` |
| Completion-write backoff holds the pool lock | One stalled reader freezes every worker's completion for the full timeout | `pipe_write_lock` was held across `write_ticket`'s sleep | Sleep outside the lock; the lock covers one syscall attempt |
...
- **ABI surface**: Exactly 17 exports (`gusset_abi_fields`, `gusset_abi_layout`, `gusset_init`, `gusset_shutdown`, `gusset_handle_open`, `gusset_handle_close`, `gusset_submit`, `gusset_take`, `gusset_cancel`, `gusset_cancel_all`, `gusset_handle_ring`, `gusset_ring_release`, `gusset_status_free`, `gusset_alloc_stats`, `gusset_drain_logs`, `gusset_buf_alloc`, `gusset_buf_free`). Verified against `internal/ffi/exports.txt` via `llvm-nm`. `gusset_abi_fields` is the field-offset table; it is not folded into `AbiLayout`, which stays 36 bytes so a version-2 caller is not overwritten.
