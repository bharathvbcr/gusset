...
- [x] `cbindgen.toml` with `language = "C"`, `include_guard`, `cpp_compat = true`; header generated by `build.rs`, committed, and diffed in CI
- [x] `Cargo.toml`: `crate-type = ["staticlib"]`, `[profile.*] panic = "unwind"`, `rust-version`, `codegen-units = 1`, `lto = "fat"` in release, `-C force-frame-pointers=yes` in `.cargo/config.toml`
...
- [x] Cross builds via `cargo zigbuild` + `CC="zig cc -target <triple>"` for Go; matrix: `linux/amd64`, `linux/arm64`, `linux/arm64-musl`, `darwin/arm64`, `windows/amd64-gnu`
